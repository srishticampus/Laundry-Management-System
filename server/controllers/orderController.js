...
const addOrder = async (req, res) => {
  try {
    const { totalAmount, shopId, custId } = req.body;
...
const addAddressOrderById = async (req, res) => {
  ...
  const { city, district, houseName, pincode, street, landmark } = req.body;
  await Order.findByIdAndUpdate(
    { _id: req.params.id },
    {
      city,
...
const viewAllServiceOrders = async (req, res) => {
  try {
    const serviceOrders = await orderServiceModel
      .find()
...
const viewAllOrderforAgent = (req, res) => {
  Order.find({
    serviceStatus: { $in: ["Requested Pickup", "Reschedule Pickup"] },
    agentStatus: false,
  })
    .populate("custId")
    .populate("shopId")
...
const viewAllDropOrderforAgent = (req, res) => {
  Order.find({
    serviceStatus: { $in: ["Requested Drop", "Reschedule Drop"] },
    dropStatus: false,
  })
    .populate("custId")
    .populate("shopId")
...
const viewAllCompletedOrdersByAGIdPickUp = (req, res) => {
  Order.find({
    agentId: req.params.id,
    serviceStatus: { $ne: "Requested Pickup" },
  })
    .sort({ createdAt: -1 })
    .populate("custId")
    .populate("shopId")
...
const updateOrderByIdonPickupIssue = async (req, res) => {
  ...
  const { city, district, street, landmark, pincode, houseName, pickupDate } =
    req.body;
  await Order.findByIdAndUpdate(
    { _id: req.params.id },
    {
