...
const registerShop = async (req, res) => {
  try {
    ...
    if (existingShopByRegNo) {
      return res
        .status(409)
        .json({ msg: "Register Number Already Registered!" });
    }
    ...
    res
      .status(200)
      .json({ msg: "Shop Registered Successfully", data: newShop });
  } catch (error) {
    console.error(error);
  ...
  shop
    ...
    .then((user) => {
      ...
      if (!user.isActive) {
        return res.json({
          status: 405,
          msg: "You are currently deactivated By Admin !!",
        });
      }
...
const viewShopById = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ msg: "Shop retrieved successfully", data: shopData });
  } catch (error) {
...
const editShopById = async (req, res) => {
  const { regNo, name, district, pincode, contact, email, location, owner } =
    req.body;
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Shop updated successfully", data: updatedShop });
  } catch (error) {
...
const deleteShopById = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ msg: "Shop deleted successfully", data: deletedShop });
  } catch (error) {
...
const toggleShopActivation = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        msg: `Shop ${
          updatedShop.isActive ? "activated" : "deactivated"
        } successfully`,
        data: updatedShop,
      });
  } catch (error) {
