...
function AgentDropReqs() {
  ...
  const fetchData = async () => {
    try {
      const result = await viewCount('viewAllDropOrderforAgent');
      if (result.success) {
  ...
  const approve = async (id) => {
    try {
      const result = await resetPassword({agentId:localStorage.getItem('agent')},'approvedropOrderByAgent', id);
      if (result.success) {
