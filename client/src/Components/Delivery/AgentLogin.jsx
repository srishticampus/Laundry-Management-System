...
function AgentLogin() {
        ...
        const handleLogin = async (e) => {
            ...
            try {
                const result = await login(data, 'agentLogin');
    
                if (result.success) {
                    console.log(result);
                    localStorage.setItem("agent", result.user._id);
...
<p className="cust-login-forgot">Forgot password</p>
                            ...
                            Don’t have an account?<span className="cust-login-link"><Link className="cust-login-link2" to='/agent-signup'> Sign Up</Link></span></p> </div>
