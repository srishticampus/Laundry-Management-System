...
function AdminDashboard() {
  ...
  const completedOrders = orders.filter(
    (order) => order.serviceStatus === "Process Completed"
  ).length;
  ...
  const data = {
    labels: ["Total Orders Processed", "Pending", "Completed", "Others"],
    datasets: [
      {
        label: "Order Analysis",
        data: [totalOrders, pendingOrders, completedOrders, otherOrders],
  ...
  return (
    <div className="admin-dashboard">
      ...
      <h4 className="mt-3">
        <center>Total Revenue Generated</center>
      </h4>
      <div
        className="chart-row"
        style={{
